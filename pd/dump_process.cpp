...
dump_process::dump_process(DWORD pid, pe_hash_database* db, PD_OPTIONS* options, bool quieter)
{
	...
	if( _ph != NULL )
	{
		...
		else
		{
			...
			{
				if (GetLastError() == 299)
					fprintf(stderr, "ERROR: - snapshot process PID 0x%x. This can be as a result of the process being a 64 bit process and this tool is running as a 32 bit process, or the process may have not finished being created or already closed.\n", pid);
				else
	...
	if( header->process_sections() )
	{
		if( header->somewhat_parsed() )
		{
			if( header->process_import_directory() )
			{
				...
				{
					if( _options->Verbose )
							printf(" preparing shitty disk '%s' at %llX\n", header->get_name(), (__int64) base);
					if( header->process_disk_image(&this->_export_list, this->_db_clean ) )
					{
						...
						// Dump the module
						printf(" ball-dumper '%s' at %llX to '%s'\n", extension, (__int64) base, filename);
